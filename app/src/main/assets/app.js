...
/* ========== ASS 字幕样式弹窗(设置页"弹窗设置"按钮) ========== */
function openSubSettings() {
  const old = document.getElementById('subModal'); if (old) old.remove();
  const s = state.settings || {};
  const m = document.createElement('div');
  m.id = 'subModal'; m.className = 'modal-mask';
  m.innerHTML = `<div class="modal-card sub-card">
    <div class="modal-title">ASS 弹幕字幕样式</div>
    <div class="sub-row"><span>滚动速度</span>${segHtml('subSpeed', s.subSpeed||'normal', [{v:'slow',label:'慢'},{v:'normal',label:'标准'},{v:'fast',label:'快'}])}</div>
    <div class="sub-row"><span>字号</span>${segHtml('subSize', s.subFontSize||26, [{v:'22',label:'小'},{v:'26',label:'标准'},{v:'30',label:'大'}])}</div>
    <div class="sub-row"><span>轨道数</span>${segHtml('subTracks', s.subTracks||6, [{v:'4',label:'4'},{v:'6',label:'6'},{v:'8',label:'8'}])}</div>
    <div class="sub-row"><span>显示昵称</span>${segHtml('subName', s.subShowName?'on':'off', [{v:'on',label:'显示'},{v:'off',label:'隐藏'}])}</div>
    <div class="sub-row"><span>内容</span>${segHtml('subContent', s.subContentAll?'all':'danmu', [{v:'danmu',label:'仅弹幕'},{v:'all',label:'全部(礼物/SC/上舰)'}])}</div>
    <div class="sub-row"><span>颜色</span>${segHtml('subColor', s.subWhiteColor?'white':'original', [{v:'original',label:'跟随原色'},{v:'white',label:'统一白色'}])}</div>
    <div class="sub-row"><span>字体</span><input id="subFontInput" class="sw-input-txt" value="${esc(s.subFont||'')}" placeholder="Microsoft YaHei"></div>
    <div class="btn-row">
      <button class="btn btn-primary" onclick="saveSubSettings()">保存</button>
      <button class="btn btn-sec" onclick="closeSubModal()">取消</button></div>
  </div>`;
  document.body.appendChild(m);
  document.getElementById('subFontInput').focus();
}
function closeSubModal() { const m = document.getElementById('subModal'); if (m) m.remove(); }
function saveSubSettings() {
  try {
    const s = state.settings || {};
    const sg = id => { const el = document.getElementById(id); if (!el) return ''; const b = el.querySelector('.seg-btn.on'); return b ? b.dataset.v : ''; };
    const updated = Object.assign({}, s, {
      subSpeed: sg('subSpeed') || s.subSpeed || 'normal',
      subFontSize: Number(sg('subSize')) || s.subFontSize || 26,
      subTracks: Number(sg('subTracks')) || s.subTracks || 6,
      subShowName: sg('subName') === 'on',
      subContentAll: sg('subContent') === 'all',
      subWhiteColor: sg('subColor') === 'white',
      subFont: (document.getElementById('subFontInput').value || '').trim(),
    });
    const r = JSON.parse(AndroidBridge.setSettings(JSON.stringify(updated)));
    toast(r.msg, r.code<0?'err':'ok');
    if (r.code > 0) { state.settings = updated; updateStatusbar(); }
    closeSubModal();
